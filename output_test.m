...
% 提取增量和衰减参数（来源于总光子数）
Total_filename = 'C:\Users\Lenovo\Desktop\毕设\光子计数探测器谱线失真机理及校正方法研究\正向仿真\fsc250331\fsc250331\UV9400\UV9400.xlsx';
x0 = 200;
...
Probability = (fitParams_a * Current_Value^3 + fitParams_b * Current_Value^2 + fitParams_c * Current_Value + fitParams_d)/(tangentParams_m * Current_Value + tangentParams_e);
prob_take_1 = 1 - 1.34*(1-Probability)
prob_take_2 = prob_take_1+(1 - prob_take_1)*0.9;%0.81
prob_take_3 = 1.00;
...
%% 数据读取
real_data = readtable('compare.xlsx', 'Range', 'A2:T513');  % 根据实际列范围调整BO
...
condition1 = (kl_value < 0.8) && (p_ks > 0.05);
...
if condition1 && condition2 
    ...
    % 假设每次循环生成的数据如下（此处仅为示例，需替换成实际数据）
    var1 = Current_Value;      % 第一行的变量（示例）
    array_data = pulse_array;            % 示例的 1x512 数组
    var2 = kl_value;                     % 第514行的变量（示例）
    var3 = p_ks;                     % 第515行的变量（示例）
    
    % 将数据整理成 515x1 的列单元格数组
    column_data = cell(515, 1);
    column_data{1} = var1;
    column_data(2:513) = num2cell(array_data');
    column_data{514} = var2;
    column_data{515} = var3;
    
    % 计算列字母（如第1列对应'A'，第2列对应'B'）
    column_letter = get_column_letter(col); % 获取正确的列字母
    range = [column_letter '1:' column_letter '515'];  % 如'A1:A515'
    ...
    % 写入Excel（直接按列写入）
    xlswrite('Simudata_UV9400_600.xlsx', column_data, sheet, range);
